...
interface AdminSidebarProps {
  activeTab: 'courses' | 'config' | 'system' | 'chats' | 'errors' | 'users';
  setActiveTab: (tab: 'courses' | 'config' | 'system' | 'chats' | 'errors' | 'users') => void;
  handleLogout: () => void;
}
    ...
    <aside className="fixed left-0 top-0 h-full w-64 bg-white border-r border-zinc-200 hidden lg:flex flex-col z-10">
      ...
      <nav className="flex-1 p-4 space-y-1">
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeTab === 'users' ? 'bg-elitc-gold/10 text-elitc-gold font-semibold shadow-sm' : 'text-zinc-500 hover:bg-zinc-50'}`}
        >
          <Database className="w-5 h-5" />
          User Management
        </button>
