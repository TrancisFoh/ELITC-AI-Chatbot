            ...
            {replies.map((reply) => {
              return (
                <motion.button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-50 border border-zinc-200 rounded-xl text-[11px] font-bold text-zinc-600 hover:border-elitc-gold hover:text-elitc-gold hover:shadow-sm transition-all active:scale-95 text-left"
                >
                  <Sparkles className="w-3 h-3 text-elitc-gold shrink-0" />
                  <span>{reply}</span>
                </motion.button>
              );
            })}
